...
const fetchShirts = async () => {
  ...
  try {
    ...
    shirts.value = data.products;
  } catch (e) {
    console.error('Failed to fetch shirts:', e);
  } finally {
    isLoading.value = false;
  }
};

watch(searchTerm, () => { clearTimeout(debounceTimer); debounceTimer = setTimeout(fetchShirts, 300); });
watch(sortBy, fetchShirts);
onMounted(fetchShirts);
</script>
  ...
  <div class="shop-page">
    ...
    <div class="filter-bar">
      <div class="search-wrap">
        ...
        <input type="text" v-model="searchTerm" placeholder="Search shirts..." class="search-input" />
      </div>
    ...
    <div v-else>
      ...
      <div v-if="shirts.length === 0" class="empty-state">
        <p>No shirts found.</p>
      </div>
    </div>
  </div>
